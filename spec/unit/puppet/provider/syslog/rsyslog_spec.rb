...
describe provider_class, :if => valid_lens? do
  ...
  context "with full file" do
    ...
    it "should list instances" do
      ...
      inst[8].should == {:name=>"news.crit /var/log/spooler", :ensure=>:present, :facility=>"news", :level=>"crit", :no_sync=>:false, :action_type=>"file", :action=>"/var/log/spooler", :action_port=>:absent, :action_protocol=>:absent}
      inst[9].should == {:name=>"local7.* /var/log/boot.log", :ensure=>:present, :facility=>"local7", :level=>"*", :no_sync=>:false, :action_type=>"file", :action=>"/var/log/boot.log", :action_port=>:absent, :action_protocol=>:absent}
    end
